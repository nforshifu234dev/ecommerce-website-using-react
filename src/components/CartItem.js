...
const CartItem = ({item}) => {
  ...
  return <div className='flex gap-x-4 py-2 lg:px-4 border-b border-gray-200 w-full font-light
   text-gray-500 '>
    <div className='w-full min-h-[150px] flex items-center gap-x-4'>
      {/* image */}
      <Link to={`/product/${id}`}>
        <img className='max-w-[80px]' src={images[0]} alt={title} />
      </Link>
      ...
      <div className='w-full flex flex-col'>
        ...
        <div className='flex gap-x-2 h-[36px] text-sm'>
          ...
          <div className='flex flex-1 max-w-[100px] items-center h-full border text-primary font-medium'>
            ...
            {/* amount */}
            <div className='h-full flex items-center px-2'>{amount}</div>
